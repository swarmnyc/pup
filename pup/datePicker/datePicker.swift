...
class DatePickerDialog: UIView {
    ...
    private func close() {
        ...
        let startRotation = (self.valueForKeyPath("layer.transform.rotation.z") as? NSNumber)
        let rotation = CATransform3DMakeRotation(0.0,0.0,0.0,0.0)
    ...
    private func createContainerView() -> UIView {
        ...
        println(self.datePickerMode)
        self.datePicker.date = self.defaultDate
        if (self.datePicker.datePickerMode == .Date) {
            self.datePicker.minimumDate = NSDate()
            self.datePicker.maximumDate = self.datePicker.minimumDate!.dateByAddingDays(14)
        }
