...
public class ENSideMenu : NSObject {
    ...
    public var menuWidth : CGFloat = UIScreen.mainScreen().bounds.size.width * 0.9 {
        didSet {
    ...
    public convenience init(sourceView: UIView, menuTableViewController: UITableViewController, menuPosition: ENSideMenuPosition) {
        ...
        var image:UIImage = UIImage(named: "iconWithText")!
        logo.image = image
        ...
        self.menuTableViewController.tableView.frame.origin.y += UIScreen.mainScreen().bounds.size.height * 0.25;
        self.menuTableViewController.tableView.frame.size.height -= UIScreen.mainScreen().bounds.size.height * 0.25;

        self.menuTableViewController.tableView.autoresizingMask = .FlexibleHeight | .FlexibleWidth
        sideMenuContainerView.addSubview(logo)
        sideMenuContainerView.addSubview(self.menuTableViewController.tableView)

        logo.snp_makeConstraints{(make) -> Void in
            make.centerX.equalTo(sideMenuContainerView)
            make.width.equalTo(UIScreen.mainScreen().bounds.size.height * 0.2)
            make.top.equalTo(sideMenuContainerView).offset(UIScreen.mainScreen().bounds.size.height * 0.125)
            make.height.equalTo(UIScreen.mainScreen().bounds.size.height * 0.2)
    ...
    private func setupMenuView() {
        ...
        sideMenuContainerView.backgroundColor = UIColor(rgba: colors.lightGray)
